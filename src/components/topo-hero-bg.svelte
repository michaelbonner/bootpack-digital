<script lang="ts">
  ...
  function sampleGeometry(tilePaths: SVGPathElement[]) {
    return tilePaths.map((el) => {
      ...
      const count = Math.max(
        16,
        Math.min(600, Math.round(length / SAMPLE_STEP)),
      );
  ...
  function frame(time: number) {
    ...
    const inHero =
      pointerSeen && mx > 0 && my > 0 && mx < rect.width && my < rect.height;
    ...
    const settled =
      strength === target && (!inHero || (glowX === mx && glowY === my));
    ...
    svgEl.style.setProperty(
      "--topo-strength",
      (strength * GLOW_OPACITY).toFixed(3),
    );
...
<div
  bind:this={host}
  class="overflow-hidden absolute inset-0"
  aria-hidden="true"
>
  <div
